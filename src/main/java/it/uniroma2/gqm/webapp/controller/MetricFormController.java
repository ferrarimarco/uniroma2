...
@SessionAttributes({"currentProject","metric","currentUser","currentMetric","units","scales","availableMetrics"})
public class MetricFormController extends BaseFormController {
	...
	public String onSubmit(Metric metric, BindingResult errors, HttpServletRequest request, HttpServletResponse response, HttpSession session) throws Exception {
		...
		// Search for metrics with the same keywords
		List<String> keywords = metric.getKeywordList();
		...
		if (request.getParameter("delete") != null) {
			...
			if (metric.getMetricOwner() == null)
				metric.setMetricOwner(userManager.getUserByUsername(request
						.getRemoteUser()));

			if (metric.getScale() != null
					&& metric.getScale().getId() != null) {
				metric.setScale(scaleManager.get(metric.getScale().getId()));
			} else {
				metric.setScale(null);
			}
			if (metric.getUnit() != null
					&& metric.getUnit().getId() != null) {
				metric.setUnit(unitManager.get(metric.getUnit().getId()));
			} else {
				metric.setUnit(null);
			}
			metric.setMetricA(metric.getMetricA() != null
					&& metric.getMetricA().getId() != null ? metricManager
							.get(metric.getMetricA().getId()) : null);
			metric.setMetricB(metric.getMetricB() != null
					&& metric.getMetricB().getId() != null ? metricManager
							.get(metric.getMetricB().getId()) : null);
		...
		Boolean foundOverlappingMetrics = false;

		if(!keywords.isEmpty() && request.getParameter("delete") == null) {
			List<Metric> metricsMatchingKeywords = metricManager.findByKeywords(keywords);

			if(metricsMatchingKeywords.size() > 1) {
				foundOverlappingMetrics = true;
			}else {
				log.debug("No metrics found with the same keywords");

				foundOverlappingMetrics = false;
			}
		}

		if (!foundOverlappingMetrics) {
			resultView = getSuccessView();
		}else {
			// here should be the view that shows metrics with matching keywords
			resultView = "redirect:metricformconfirmation";
			session.setAttribute("currentMetric", metric);
		}
