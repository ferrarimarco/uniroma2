...
<head>
	<title>
		<fmt:message key="metricDetail.title" />
	</title>
...
<div class="span2">
	<h2>
		<fmt:message key='metricDetail.heading' />
	</h2>
	<p>
		<fmt:message key="metricDetail.message" />
	</p>
	<p>
		<fmt:message key="metric.goals.message" />
	</p>
	<p>
		<fmt:message key="metric.owner.message" />
	</p>
	<b>&nbsp;&nbsp;&nbsp;${metric.metricOwner.fullName}</b>

	<p>
		<fmt:message key="questionMetric.question.message" />
	</p>
	<ul>
...
<div class="controls">
	<form:select path="metricB.id" disabled="${metric.metricOwner ne currentUser && not empty metric.id}">
		<form:option value="" label="None" />
		<form:options items="${availableMetrics}" itemValue="id"
			itemLabel="code" />
	</form:select>
...
<div class="controls">
	<form:select path="satisfyingConditionOperation"
...
<c:if test="${not empty metric.id}">
	<spring:bind path="metric.questions">
		<div class="control-group${(not empty status.errorMessage) ? ' error' : ''}">
			...
			<div class="controls">
				<select id="questions" name="questions" multiple="multiple"
					style="width: 500px;">
				...
				</select> * <br /> * This list contains the questions associated to the goal
				for which the user logged in is Measurement Model Design Manager.
			</div>
	</spring:bind>
</c:if>
